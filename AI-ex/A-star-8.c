...
int find_closet(int res,int result[_MAX_PATH],int pcount,int path[_MAX_PATH][N][N],int aim_board[N][N]);
...
int main()
{
    ...
    while(check(pre_board,aim_board) !=0 )//循环移动空点添加新的路径点直到花费最小的点
    {
        ...
        re = find_closet(res,result,pcount,path,aim_board);
        copy_board(pre_board,path[re]);
